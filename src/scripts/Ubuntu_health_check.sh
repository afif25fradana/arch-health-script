...
show_usage() {
    cat <<EOF
Usage: $0 [options]
  -f, --fast        Skip slower checks (like debsums).
...
load_configuration() {
    ...
    load_config_array "$config_file" "deductions" "failed_services:25,unclaimed_devices:10,upgradable_pkgs:1,orphans:5,changed_configs:2,syslog_errors:5"
...
main() {
    ...
    echo -e "${BLUE}=== Kicking off the Ubuntu/Debian Health Check ===${NC}"
    ...
    if [[ -n "$missing_pkgs" ]]; then
        ...
        log_info "You can do this by running: ${GREEN}sudo apt update && sudo apt install ${missing_pkgs}${NC}"
    fi
}
